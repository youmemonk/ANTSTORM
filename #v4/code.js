...
function doGet(request) {
  ...
  // var logsSheet = '16DA19UwwwjEFzheuxCgaOOkdKSIF_8skHinOgaN-EWk'
  ...
  function rootFolderMapping() {
    ...
    try {
      ...
      // var logsbook = SpreadsheetApp.openById(logsSheet);
      // var logsSheetData = logsbook.getSheetByName(logsSheetName);
      // var repoLogsData = arrayToJSONObject(logsSheetData.getDataRange().getValues());
      ...
      for(var i=0; i<repoLogsData.length; i++){
        newRepoLogsData[i]={'Folder ID':repoLogsData[i]['Folder ID'], 'Opportunity ID':repoLogsData[i]['Opportunity ID'], 'Domain Name': repoLogsData[i]['Domain Name']}
      }
      // Logger.log([repoLogsData[0]['Folder ID'], repoLogsData[0]['Opportunity ID'], repoLogsData[0]['Domain Name']])
      // Logger.log(newRepoLogsData)
